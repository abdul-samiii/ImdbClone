...
const YourRatings = () => {
  ...
  const FreeTrailerData = useSelector(item => item?.ReviewReducer?.userRatings)
                  ...
                  return (
                    <div className='flex py-12 px-4'>
                      <div className='hidden'>
                        {
                      watchlist?.trailerId?.reviews?.map((review) => {
                        if (review?.ratings) {
                          sum += review.ratings
                        }
                      ...
                      <div className='h-full flex group'>
                        <video className='h-[100%] w-[300px] max-w-none' controls>
                          <source src={`http://localhost:5500/uploads/videos/${watchlist?.trailerId?.link}`} type='video/mp4' />
                        </video>
                      </div>
                      <p className='font-mono px-2 mt-1'>1.</p>
                      <div>
                        <h2 className='font-bold text-lg text-blue-600'>{watchlist?.trailerId?.title}</h2>
                        <p className='text-xs'>{watchlist?.trailerId?.channel?.channelName}</p>
                        ...
                        <div className='flex ml-20'>
                          ...
                          <p className=' ml-1 -mt-[1px]'>{watchlist?.trailerId?.reviews[0] ? Number(sum / watchlist.trailerId.reviews.length).toFixed(2) : 0 }{watchlist.trailerId.reviews.length > 0 && `(${watchlist.trailerId.reviews.length})` }</p>
                          <p className='text-gray-600 text-sm ml-2'>Rating</p>
                        </div>
                        <div className='flex mt-4 mb-4'>
                          { starFlag >= 1
                            ? (
                              <StarIcon
                                ...
                                onClick={() => handleAddTrailerReview(watchlist?.trailerId?._id, 1)}
                              />
                            )
                            : (
                              <StarIconO
                          ...
                          { starFlag >= 2
                            ? (
                              <StarIcon
                                className='h-10 text-[#E0B416] ml-2 cursor-pointer hover:bg-[#534b4b] rounded-md px-2 py-2 -mt-2'
                                onClick={() => handleAddTrailerReview(watchlist?.trailerId?._id, 2)}
                              />
                            )
                            : (
                              <StarIconO
                          ...
                          { starFlag >= 3
                            ? (
                              <StarIcon
                                className='h-10 text-[#E0B416] ml-2 cursor-pointer hover:bg-[#534b4b] rounded-md px-2 py-2 -mt-2'
                                onClick={() => handleAddTrailerReview(watchlist?.trailerId?._id, 3)}
                              />
                            )
                            : (
                              <StarIconO
                          ...
                          { starFlag >= 4
                            ? (
                              <StarIcon
                                className='h-10 text-[#E0B416] ml-2 cursor-pointer hover:bg-[#534b4b] rounded-md px-2 py-2 -mt-2'
                                onClick={() => handleAddTrailerReview(watchlist?.trailerId?._id, 4)}
                              />
                            )
                            : (
                              <StarIconO
                          ...
                          { starFlag === 5
                            ? (
                              <StarIcon
                                className='h-10 text-[#E0B416] ml-2 cursor-pointer hover:bg-[#534b4b] rounded-md px-2 py-2 -mt-2'
                                onClick={() => handleAddTrailerReview(watchlist?.trailerId?._id, 5)}
                              />
                            )
                            : (
                              <StarIconO
